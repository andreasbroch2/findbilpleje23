...
export async function fetchAPI(query = '', { variables }: Record<string, any> = {}) {
  ...
  const res = await fetch('https://findbilpleje.dk/graphql', {
    headers,
    method: 'POST',
    body: JSON.stringify({
      query,
...
export async function addCountMutation(id, fieldName) {
  const data = await fetchAPI(`
  mutation ADD_COUNT_MUTATION( $id: ID!, $fieldName: String! ) {
    addCount(input: {fieldName: $fieldName, id: $id}) {
      newCount
    }
  }
  `,
    {
      variables: { id, fieldName },
    })
...
export async function getSingleDetailer(slug) {
  const data = await fetchAPI(`
  query GET_DETAILER {
	  detailer(id: "${slug}", idType: SLUG) {
      content
      slug
      id
      databaseId
