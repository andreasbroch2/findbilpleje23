...
import { getACFHomepage, getAllDetailers, getAllPostsForHome, getNavMenu, getSinglePage } from '../lib/api'
    ...
    <Layout data={homePage}>
      ...
      <Container>
        <div className='entry-content homepage'>
          ...
          <section className='wp-block-cover alignfull hero-section'>
            ...
            <div className='max-w-content z-40'>
              <div className='md:flex gap-8 items-center py-8'>
                <div className='wp-block-column is-layout-flow basis-1/2'>
                  <h1 className='text-white mb-8'>{data.homePage.heroHeadline}</h1>
                  <p className='text-white'>{data.homePage.heroDescription}</p>
                </div>
                <div className='wp-block-column is-layout-flow basis-1/2 text-center text-white flex flex-col item-space-between'>
                  <ContactForm />
                </div>
              </div>
            </div>
          ...
          <section>
            <h2 className='mt-8'>Populære Bilplejere</h2>
            <PostGrid posts={allPosts?.edges ?? []} />
          </section>
          <TextUSPBox />
          <section>
            <h2 className='mt-8'>Populære Tjenester</h2>
